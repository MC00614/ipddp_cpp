...
def visualize_trajectory_3d(xc, uc, N, T_max, cmap='viridis'):
    ...
    segments = [np.column_stack([xc[0, k:k+4],
                                 xc[1, k:k+4],
                                 xc[2, k:k+4]]) for k in range(N-3)]
    ...
    anim = animation.FuncAnimation(fig, animate, frames=N, interval=60, blit=False)
    
    # Add a colorbar for the speed
