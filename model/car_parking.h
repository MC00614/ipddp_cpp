...
CarParking::CarParking() {
    ...
    q = [this](const Eigen::VectorXd& x, const Eigen::VectorXd& u) -> double {
        ...
        return 1e-2 * u0*u0 + 1e-4*u(1)*u(1) +
            1e-3 * (std::sqrt(x(0) * x(0) + 1e-3) - 1e-3) +
            1e-3 * (std::sqrt(x(1) * x(1) + 1e-3) - 1e-3);
    };

    // Terminal Cost Function
    p = [this](const Eigen::VectorXd& x) -> double {
        return 0.1 * (std::sqrt(x(0) * x(0) + 0.01) - 0.01) +
           0.1 * (std::sqrt(x(1) * x(1) + 0.01) - 0.01) +
           1.0 * (std::sqrt(x(2) * x(2) + 0.01) - 0.01) +
           0.3 * (std::sqrt(x(3) * x(3) + 1.0) - 1.0);
    };
}
