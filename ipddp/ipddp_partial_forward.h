...
void IPDDP::init(Param param) {
    ...
    for (int i = 0; i <= MAX_STEP; ++i) {
        ...
        std::cout<<std::pow(2.0, static_cast<double>(-i))<<std::endl;
    }
}
...
void IPDDP::solve() {
    ...
    while (iter++ < this->param.max_iter) {
        ...
        std::cout<< "Step Size : " << step_list[result_step] << std::endl;
...
void IPDDP::checkRegulate() {
    ...
    // else if (result_step == 0) {--regulate;}
    // else if (result_step <= 3) {regulate = regulate;}
    else {--regulate;}
...
void IPDDP::forwardPass() {
    ...
    for (int step = 0; step < MAX_STEP; ++step) {
        ...
        for (int t = 0; t < model->N; ++t) {
            ...
            for (int i = 0; i < model->dim_hs.size(); ++i) {
                ...
                < (1 - tau)*(1 - tau) * (Y.col(t).row(dim_hs_top[i]).array().pow(2.0) - Y.col(t).middleRows(dim_hs_top[i]+1, model->dim_hs[i]-1).array().pow(2.0).sum())).any())
                {max_slack_step_failed = true; break;}
            }
    ...
    for (int slack_step = max_slack_step; slack_step < MAX_STEP; ++slack_step) {
        ...
        for (int t = 0; t < model->N; ++t) {
            int t_dim_x = t * model->dim_x;
            Y_new.col(t) = Y.col(t) + (slack_step_size * ky.col(t)) + (Ky.middleCols(t_dim_x, model->dim_x) * (X_new.col(t) - X.col(t)));
            U_new.col(t) = U.col(t) + (slack_step_size * ku.col(t)) + (Ku.middleCols(t_dim_x, model->dim_x) * (X_new.col(t) - X.col(t)));
            X_new.col(t+1) = model->f(X_new.col(t), U_new.col(t)).cast<double>();
        }
        ...
        for (int dual_step = 0; dual_step < MAX_STEP; ++dual_step) {
            ...
            for (int t = 0; t < model->N; ++t) {
                int t_dim_x = t * model->dim_x;
                S_new.col(t) = S.col(t) + (dual_step_size * ks.col(t)) + (Ks.middleCols(t_dim_x, model->dim_x) * (X_new.col(t) - X.col(t)));
            }
            inner_failed = false;
            for (int t = 0; t < model->N; ++t) {
                if (model->dim_g) {
                ...
                for (int i = 0; i < model->dim_hs.size(); ++i) {
                    ...
                    < (1 - tau)*(1 - tau) * (S.col(t).row(dim_hs_top[i]).array().pow(2.0) - S.col(t).middleRows(dim_hs_top[i]+1, model->dim_hs[i]-1).array().pow(2.0).sum())).any())
                    {inner_failed = true; break;}
                }
